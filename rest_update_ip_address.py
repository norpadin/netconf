...
def configure_ip_address(interface, ip):
    ...
    url = url_base + "/data/ietf-interfaces:interfaces/interface={i}".format(
        i=interface)
...
def interface_selection(interfaces):
    ...
    while sel == args.interface or not sel in [
            intf["name"] for intf in interfaces]:
...
def main():
    ...
    print("The device has the following interfaces: \n")
