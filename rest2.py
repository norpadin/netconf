...
class IosXeRest(object):
    ...
    def get_hostname(self):
        return self.make_call(
            'GET', 'Cisco-IOS-XE-native:native/', 'hostname').json()
    ...
    def set_hostname(self, hostname):  # Change HOSTNAME JSON
        ...
        if self.make_call('PUT', 'Cisco-IOS-XE-native:native/', 'hostname',
                          payload).status_code == 204:
            return True

    # Return the interfaces in JSON format
    def get_interface(self):
        return self.make_call('GET', 'ietf-interfaces:interfaces').json()[
    ...
    def configure_interface_ip(self, interface, ip):
        # build RESTCONF URI
        uri = "Cisco-IOS-XE-native:native/interface/{i}/ip/address/primary".format(
            i=interface)
        # print("\n the following RESTCONF URI will be used to configure the IP address on the requested interface : " + self.ip +uri + "\n")

        # build JSON payload
        payload = "{\"primary\": {\"address\": \""+str(
            ip["address"])+"\", \"mask\": \""+str(ip["mask"])+"\"}}"
    ...
    def configure_interface_description(self, interface, description):
        uri = "Cisco-IOS-XE-native:native/interface/{i}/description".format(
            i=interface)
    ...
    def configure_vlan(self, vlan):
        uri = "Cisco-IOS-XE-native:native/vlan/Cisco-IOS-XE-vlan:vlan-list"
        payload = "{\"Cisco-IOS-XE-vlan:vlan-list\": {\"id\": \""+str(
            vlan["id"])+"\", \"name\": \""+str(vlan["name"])+"\"}}"
        call = self.make_call('PATCH', uri, "", payload)
        print(call)
        if call.status_code == 204:
            return True
...
class GuiShell(IosXeRest):
    ...
    def pretty_get_hostname(self):
        ...
        print("The current hostname of the device is '{h}'.".format(
            h=hostname['Cisco-IOS-XE-native:hostname']))
    ...
    def pretty_get_interfaces(self):
        ...
        for item in interfaces:
            if item['ietf-ip:ipv4'] or item['ietf-ip:ipv6']:
                ...
                print(
                    "     IP address : " +
                    str(item['ietf-ip:ipv4']['address'][0]['ip']) +
                    "\n     Netmask : " +
                    str(item['ietf-ip:ipv4']['address'][0]['netmask']))
    ...
    def pretty_configure_interface(self):
        ...
        if (self.configure_interface_description(interface_name, description)):
            print(
                'interface ' + sel_int +
                ' has successfully been configured with IPv4: ' +
                ip["address"] + "\n")
    ...
    def pretty_configure_vlan(self):
        ...
        if self.configure_vlan(vlan):
            print("The VLAN {v} with name {n} has successfully been configured".format(
                v=vlan["id"], n=vlan["name"]))
